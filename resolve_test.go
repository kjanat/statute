...
func TestResolveParseErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{"transport.response_header_timeout", withPool(Pool{Backends: okBackends, Strategy: RoundRobin, Transport: Transport{ResponseHeaderTimeout: bad}}), "response_header_timeout:"},
		{"transport.response_header_timeout_negative", withPool(Pool{Backends: okBackends, Strategy: RoundRobin, Transport: Transport{ResponseHeaderTimeout: "-1s"}}), "response_header_timeout:"},
