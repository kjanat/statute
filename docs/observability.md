...
So `Statuses("200-299")` really does suppress 500s — errors **within the selected ranges** are never sampled out, and everything outside the ranges is never logged at all. Filtering applies to the final status: the recorder ignores 1xx interim responses, so a 103 → 404 exchange filters as 404.
